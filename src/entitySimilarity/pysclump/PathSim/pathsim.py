...
class PathSim:
    def __init__(self, type_lists, incidence_matrices, similarity_matrices={}):
    ...
    def compute_similarity_matrix(self, metapath):
        ...
        for (index1, index2) in product(range(num_nodes), range(num_nodes)):
            if index1 != index2:
                ...
                if (num_paths_11 + num_paths_22) > 0:
                    ...
                    if similarity_matrix[index1][index2] > maxDist:
                        maxDist = similarity_matrix[index1][index2]
                        print(f'new max {maxDist}')
                        print(num_paths_12, num_paths_11, num_paths_22)
                        print(index1, index2)
                    elif similarity_matrix[index1][index2] < minDist:
        ...
        return similarity_matrix, metapath_matrix
    ...
    def compute_metapath_matrix(self, metapath):
        ...
        for node_type, next_node_type in zip(metapath[:metapath_length//2], metapath[1:metapath_length//2 + 1]):
            ...
            print(curr_matrix.shape, self.incidence_matrices[incidence_type].shape)
